...
def handle_update(update):
    ...
    if update_id is not None and db.already_processed(update_id):
        print(f"[handlers] skipping duplicate update_id={update_id} (Telegram retry)")
        return
...
def do_broadcast(chat_id, text):
    ...
    tg.send_message(chat_id, f"📢 Broadcasting to {len(user_ids)} users in the background...")
